...
void ShadowShader::UpdateShadows(Camera camera, Transform trans)
{
	...
	Camera cam;
	cam.SetPosition(glm::vec3(((SIZE_X / 2)), ((SIZE_Y / 2))- 10, -51.2f));
	float temp = 32;
	m_shader.Bind();//fel
	m_shader.Update(trans, camera);
	glm::mat4 lightProjection, lightView;
	GLfloat nearPlane = 1.0f, farPlane = 750.5f;
	lightProjection = glm::ortho(-temp - (SIZE_X / 2), temp - (SIZE_X / 2), -18.0f + ((SIZE_Y / 2)), 18.0f + ((SIZE_Y / 2)), nearPlane, farPlane);
