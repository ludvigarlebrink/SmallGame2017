...
void VirtualKeyboard::Init()
{
	...
	m_background.SetColor(104, 86, 58, 180);
	m_background.SetSize(600, 440);

	m_background2.SetColor(175, 148, 107, 180);
	...
	m_title.SetSize(60);
	m_title.SetPosition(0, 145);
	m_title.SetFont(".\\Assets\\Fonts\\Snap.ttf");
	...
	m_help[4].SetText("RB LAYOUT");
	m_help[5].SetText("S DONE");

	for (size_t i = 0; i < NUM_HELP; i++)
	{
		m_help[i].SetPosition(440, (i * -40) + 20 );
		m_help[i].SetFont(".\\Assets\\Fonts\\Snap.ttf");
		m_help[i].SetColor(235, 235, 180, 255);
		m_help[i].SetSize(26);
	}
	...
	m_chars[m_pos].SetColor(229, 122, 16, 255);
	m_chars[m_pos].SetSize(m_hSize);
}
...
void VirtualKeyboard::Input()
{
	if (m_input->GetButtonDown(CONTROLLER_BUTTON_DPAD_UP))
	{
		if (m_pos - 10 >= 0)
		{
			m_chars[m_pos].SetColor(235, 235, 180, 255);
...
void VirtualKeyboard::SwitchLayout(int layout)
{
	...
	for (size_t i = 0; i < NUM_CHARS; i++)
	{
		...
		m_chars[i].SetFont(".\\Assets\\Fonts\\Snap.ttf");
		m_chars[i].SetColor(235, 235, 180, 255);
	}
